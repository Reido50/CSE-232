/*  Reid Harry
    ...
    a few local stores, so I can track what items they have in stock and how much they are selling things for. With that information 
    (and knowing what I need to purchase), I can assemble a list of what I need to purchase from each location.
    ...
    After the store information is my shopping list. For each item, I want to get the best deal possible, even if that means going to 
    different stores.

    For example, if I want to buy 15 pieces of "Candy", I would need to go to the store with the cheapest Candy (the "Local Grocery") 
    and buy everything they have (4 pieces), then go to the next cheapest place ("Sparty's") and buy them out as well (10 pieces). I 
    still need one more piece of candy, so I need to go to "Corner Store". I need to determine how much all that candy costs 
    (4 * $0.51 + 10 * $0.85 + 1 * $1.00 is $11.54)
    ...
    All the code for this project should be in a file named "project.cpp". Be sure to write clean, clear code, and be sure to use 
    structs, algorithms, or any other tools we've taught you if would be useful. Remember to structure your code into clearly commented, 
    short functions, use const and references where appropriate, and make sure that your code doesn't generate any warnings from the compiler.

    Hints:

    Be careful with rounding the numbers representing the prices, I need to know the costs to the exact penny. You might consider 
    storing the prices in a long (as the number of cents). Example $4.50 is stored as (long) 450, so that there isn't any loss of precision 
